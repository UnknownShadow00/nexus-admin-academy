  ...
  if (contacts.length === 0) {
    ...
    return (
      <Card className="flex min-h-64 flex-col items-center justify-center px-5 py-10 text-center">
        <EmptyIcon aria-hidden="true" className="h-9 w-9 text-text-muted" />
        <h2 className="mt-4 text-base font-bold text-text">{emptyTitle}</h2>
        ...
        {contacts.map((contact) => {
          ...
          return (
            <button
              ...
              type="button"
            >
              <span className="relative flex h-9 w-9 shrink-0 items-center justify-center rounded-sm border border-border bg-surface text-text-muted">
