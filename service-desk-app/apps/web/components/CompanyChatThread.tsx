...
export function CompanyChatThread({ contact, thread }: CompanyChatThreadProps) {
  ...
  if (!contact) {
    return (
      <Card className="flex min-h-[32rem] flex-col items-center justify-center border-dashed px-6 py-10 text-center">
        <IconMessages aria-hidden="true" className="h-10 w-10 text-text-muted" />
  ...
  return (
    <Card className="flex min-h-[32rem] flex-col overflow-hidden">
      ...
      <div
        aria-live="polite"
        className="flex min-h-72 flex-1 flex-col gap-3 overflow-y-auto bg-surface/40 p-4"
      >
        {messages.length === 0 ? (
              ...
              >
                ...
                <p
                  className={`mt-1 text-[10px] font-bold uppercase tracking-wide ${
                    message.fromStudent ? 'text-accent' : 'text-text-muted'
                  }`}
                >
      ...
      {ticketId ? (
        <section className="space-y-3 border-t border-border bg-surface-raised/70 p-4">
      ...
      <form
        className="border-t border-border bg-surface-raised p-4"
