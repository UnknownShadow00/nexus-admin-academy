...
export function AssetList({
  ...
  if (isLoading) {
    return (
      <Card aria-label="Loading assets" className="divide-y divide-border">
        {Array.from({ length: 8 }, (_, index) => (
          ...
          >
            ...
            <div className="mt-2 h-3 w-48 rounded-sm bg-surface-muted/70" />
          </div>
        ))}
      </Card>
    );
  ...
  return (
    <Card className="sd-assets-layout max-h-[68vh] overflow-y-auto">
      <div className="sd-assets-table-header sticky top-0 z-10 hidden grid-cols-[1fr_1.3fr_1fr_auto] gap-3 border-b border-border bg-surface-muted px-4 py-2 text-xs font-extrabold uppercase text-text-muted sm:grid">
        ...
        {assets.map((asset) => {
          ...
          return (
            <button
              ...
              className={`sd-assets-row sd-focus-ring grid w-full gap-2 px-4 py-3 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-focus sm:grid-cols-[1fr_1.3fr_1fr_auto] sm:items-center sm:gap-3 ${
            ...
            >
              ...
              <span className="flex min-w-0 flex-wrap items-center gap-2 text-xs text-text-muted">
