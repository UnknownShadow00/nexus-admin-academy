...
import { CheckCircle2, ClipboardCheck, ExternalLink, Eye, ShieldCheck, XCircle } from "lucide-react";
...
function noteFields(workbench) {
  const additional = Array.isArray(workbench?.additional_note_fields) ? workbench.additional_note_fields : [];
  return [
    ...BASE_NOTE_FIELDS,
    ...additional
      .filter((field) => field?.id && field?.label)
      .map((field) => [field.id, field.label, field.placeholder || "Record the required handoff detail."]),
  ];
}

function restoredNotes(value, fields) {
  try {
    const parsed = JSON.parse(value || "{}");
    return Object.fromEntries(fields.map(([field]) => [field, typeof parsed?.[field] === "string" ? parsed[field] : ""]));
  } catch {
    return Object.fromEntries(fields.map(([field]) => [field, ""]));
  }
}

export default function EvidenceCaseWorkbench({ workbench, questions, feedback, initialNotes, submitted, busy, onVerify, onSubmit }) {
  ...
  const [notes, setNotes] = useState(() => restoredNotes(initialNotes, fields));
  ...
  const documentationComplete = fields.every(([field]) => (notes[field] || "").trim().length > 0);
  ...
  function recordTerminalInspection(result) {
    if (!result?.inspectionId) return;
    setOpened((current) => (current.includes(result.inspectionId) ? current : [...current, result.inspectionId]));
  ...
  return (
    <section className="min-w-0 space-y-5" aria-labelledby="evidence-case-workbench-title">
      <div className="rounded-lg border border-blue-200 bg-blue-50/60 p-4 dark:border-blue-900 dark:bg-blue-950/20">
        ...
        <h3 id="evidence-case-workbench-title" className="mt-1 text-base font-semibold text-slate-900 dark:text-white">{workbench?.title || "Evidence case workbench"}</h3>
        <p className="mt-2 text-sm text-slate-700 dark:text-slate-300">{workbench?.complaint || workbench?.brief}</p>
        {workbench?.guidance ? (
      ...
      <div className="min-w-0 rounded-lg border border-slate-200 p-4 dark:border-slate-700">
        ...
        <div className="mt-3 flex flex-wrap gap-2" role="tablist" aria-label="Evidence panels">
          {panels.map((panel) => (
            <button
              key={panel.id}
              type="button"
        ...
        {workbench?.terminal_profile ? (
          <div className="mt-4 min-w-0">
            <p className="mb-2 text-xs text-slate-500 dark:text-slate-400">Use the focused case terminal when command output is relevant. No command is prefilled.</p>
            <TerminalWidget caseProfile={workbench.terminal_profile} compact onCommandResult={recordTerminalInspection} />
          </div>
        ) : null}
      </div>
      ...
      {reinforcementScenarios.length ? (
        <div className="rounded-lg border border-slate-200 p-4 dark:border-slate-700">
          <h4 className="font-semibold text-slate-900 dark:text-white">Related Service Desk practice</h4>
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">These existing cases retain their normal unlocks and authoritative Service Desk grading.</p>
          <div className="mt-3 grid gap-2">
