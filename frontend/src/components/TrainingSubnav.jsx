...
const items = [
  { to: "/learning-path", label: "My Course", end: true },
...
export default function TrainingSubnav() {
  return (
    <nav aria-label="Course sections" className="flex max-w-full gap-1 overflow-x-auto rounded-xl border border-slate-200 bg-white p-1 dark:border-slate-700 dark:bg-slate-900">
      {items.map((item) => (
        <NavLink
          key={item.to}
          to={item.to}
