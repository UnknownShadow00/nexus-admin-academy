...
import { getTrainingProgress } from "../services/api";
...
export default function TrainingProgressPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-6 p-4 pb-20 sm:p-6">
      ...
      <section className="rounded-2xl bg-slate-950 p-5 text-white dark:bg-blue-950 sm:p-7"><div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-semibold uppercase tracking-wide text-blue-300">Overall Training Completion</p><p className="mt-2 text-4xl font-bold">{data.overall_training.percent}%</p><p className="mt-2 text-sm text-slate-300">{data.overall_training.completed} of {data.overall_training.total} required activities complete</p>{current ? <p className="mt-3 text-sm text-white">Current: Week {current.week_number} — {current.title}</p> : null}</div><Link className="inline-flex min-h-11 items-center justify-center rounded-xl bg-white px-5 py-3 font-bold text-blue-700" to="/training">Continue Training</Link></div></section>
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><Metric label="Videos Watched" metric={data.videos} Icon={PlayCircle} /><Metric label="Quizzes Completed" metric={data.quizzes} Icon={Trophy} note={`${data.quizzes.percent}% complete · ${data.quizzes.average_score_percent ?? 0}% average score`} /><Metric label="Required Practice" metric={data.practice} Icon={FlaskConical} /><Metric label="Guided Labs" metric={data.guided_labs} Icon={CheckCircle2} /><Metric label="Tickets Completed" metric={data.tickets} Icon={Ticket} /><Metric label="Weeks Completed" metric={{ completed: data.weeks_completed, total: data.total_weeks, percent: data.total_weeks ? Math.round(data.weeks_completed / data.total_weeks * 100) : 0 }} Icon={Target} /></section>
