...
export default function AdminServiceDeskPage() {
  ...
  return <main className="mx-auto max-w-6xl space-y-6 p-4 md:p-6">
    ...
    <section className="panel"><h2 className="text-xl font-bold">Scenarios and validation</h2><ul className="mt-3 grid gap-2 md:grid-cols-2">{scenarios.map((item) => <li key={item.id} className="rounded border p-3"><strong>{item.title}</strong><br /><span className="text-sm">{item.stable_key} · {item.status}</span><ul className="mt-2 space-y-1 text-sm">{item.versions?.map((version) => <li key={version.version_number}>Version {version.version_number} · {version.status} · {version.health_valid ? "Health passing" : "Health pending"}</li>)}</ul></li>)}</ul></section>
    ...
    {replay ? <section className="panel"><h2 className="text-xl font-bold">Attempt {replay.attempt.id} event replay</h2><p className="mt-2">Score: {replay.grade?.overall_score ?? 0}% · {replay.grade?.passed ? "Passed" : "Not passed"}</p>{replay.grade ? <div className="mt-3 rounded border p-3"><h3 className="font-bold">Grade breakdown</h3><dl className="mt-2 grid gap-2 text-sm sm:grid-cols-3"><div><dt className="text-slate-500">Rubric</dt><dd>{replay.grade.rubric_version}</dd></div><div><dt className="text-slate-500">Technical completion</dt><dd>{replay.grade.technical_complete ? "Complete" : "Incomplete"}</dd></div><div><dt className="text-slate-500">Critical failure</dt><dd>{replay.grade.critical_failure ? "Yes" : "No"}</dd></div></dl><p className="mt-2 text-sm"><strong>Earned scoring events:</strong> {(replay.grade.details?.earned_score_keys || []).join(", ") || "None"}</p></div> : null}<ol className="mt-3 list-decimal space-y-2 pl-5 text-sm">{replay.events.map((event) => <li key={event.sequence_number}><strong>{event.tool}</strong> · {event.event_type} · {event.success ? "accepted" : "rejected"}</li>)}</ol></section> : null}
  </main>;
}
