...
QUIZZES_D = [
    {
        ...
        "questions": [
            _q("What is an Organizational Unit (OU) primarily used for?",
               "Assigning IP addresses", "Organizing objects AND being the target that Group Policy links to",
               "Storing passwords", "Routing between VLANs",
            ...
            _q("Best practice for granting five users access to a folder is to:",
               "Grant each user directly on the folder", "Add them to a group and grant the GROUP access",
               "Make them all administrators", "Share the folder to Everyone",
            ...
            _q("A DISTRIBUTION group differs from a SECURITY group in that it:",
               "Grants NTFS permissions", "Is only an email list and cannot grant permissions",
               "Spans forests", "Is faster",
            ...
            _q("A user reports their account is disabled. The correct first action is:",
               "Re-enable it immediately", "Confirm WHY it was disabled (HR/security) before touching it",
               "Reset the password", "Delete and recreate it",
            ...
            _q("After adding a user to a new security group, they still can't access the resource. Missing step:",
               "Reboot the domain controller", "The user must sign out and back in to refresh their token",
               "Reset their password", "Disable inheritance",
            ...
            _q("The A-G-DL-P best practice means:",
               ...
               "Admins Get Direct Local Permissions", "Assign Groups Directly, Leave Permissions",
            ...
            _q("To reset a user's password in AD you should also typically:",
               "Delete their profile", "Require change at next logon and verify identity first",
               "Disable the account", "Remove them from all groups",
    ...
    },
    {
        ...
        "questions": [
            _q("The #1 reason a computer fails to join the domain is:",
               "Wrong time zone", "DNS is not pointed at the domain's DNS (the DC), so it can't find a DC",
               "Insufficient RAM", "The firewall is on",
            ...
            _q("'The trust relationship between this workstation and the primary domain failed.' Best fix:",
               "Reinstall Windows", "Repair the secure channel (Test-ComputerSecureChannel -Repair) or reset the computer account",
               "Change the user's password", "Rejoin from scratch immediately, always",
               "B", "Repair the secure channel; full unjoin/rejoin is the outdated sledgehammer."),
            _q("A disabled COMPUTER account in AD means:",
               "The user can't log in anywhere", "That machine can't authenticate to the domain",
               "The domain is down", "Group Policy is disabled",
            ...
            _q("In the A-G-DL-P model, to give a user access to the Finance share you:",
               "Grant the user NTFS rights directly", "Add the user to the Finance GLOBAL group (which is in the resource's domain-local group)",
               "Add the user to Domain Admins", "Share to Everyone",
            ...
            _q("Moving a computer account to a different OU can change:",
               "Its IP address", "Which Group Policies apply to it", "Its MAC address", "Its hostname",
            ...
            _q("'My mapped drive is missing' in a domain is often caused by:",
               "A dead hard drive", "A Group Policy drive-map not processing (not necessarily a permissions issue)",
               "A virus", "Wrong DNS",
    ...
    },
    {
        ...
        "questions": [
            _q("Group Policy application order is:",
               "OU → Domain → Site → Local", "Local → Site → Domain → OU, with later winning on conflict",
               "Random", "Domain only",
            ...
            _q("A setting expected from a domain GPO is being overridden. Most likely:",
               "The server is down", "A GPO linked closer (e.g. at the OU) wins by precedence",
               "DNS failure", "The user's password expired",
            ...
            _q("To see exactly which GPOs applied to a user/computer, run:",
               "ipconfig /all", "gpresult /r (or /h for an HTML report)", "nslookup", "sfc /scannow",
            ...
            _q("gpresult shows the GPO is NOT applied and was filtered. A common cause:",
               "The cable is bad", "The object is in the wrong OU or excluded by security filtering",
               "The monitor is off", "DNS is slow",
               "B", "Wrong OU or a security filter excluding the object are classic filtering causes."),
            _q("'Enforced' on a GPO does what?",
               "Disables the GPO", "Forces it to win regardless of normal precedence and Block Inheritance",
               "Applies only to computers", "Deletes conflicting GPOs",
            ...
            _q("A user-configuration policy setting takes effect:",
               "At computer boot", "At the user's next logon (after refresh)", "Never", "Only via reboot of the DC",
            ...
            _q("The fix when gpresult shows the object is simply in the wrong OU is usually to:",
               "Edit the GPO settings", "Move the object to the correct OU (with approval), then gpupdate/re-logon",
               "Reinstall Windows", "Reset the password",
    ...
    },
    {
        ...
        "questions": [
            _q("Why does Active Directory depend on DNS?",
               "For faster internet", "Clients locate domain controllers via DNS (SRV) records",
               "DNS stores passwords", "It doesn't",
            ...
            _q("The RIGHT way to give a printer a stable address via DHCP is:",
               "A hand-set static IP on the printer", "A DHCP reservation tying the address to the printer's MAC",
               "Turn off DHCP", "Assign it the gateway address",
            ...
            _q("Which cmdlet finds all locked-out accounts?",
               "Get-Process", "Search-ADAccount -LockedOut", "Restart-Service", "Get-Help",
            ...
            _q("Before running a bulk CHANGING command against many objects, you should:",
               "Run it and see what happens", "Run the read-only version first and/or use -WhatIf to preview",
               "Reboot the server", "Disable logging",
            ...
            _q("A DHCP scope 'is full' and new devices in that subnet get no address. This is:",
               "A DNS problem", "Scope exhaustion — the server-side view of 'a whole area gets no IP'",
               "A cable fault", "A password issue",
    ...
    },
    {
        ...
        "questions": [
            _q("The single most important thing about a backup is:",
               "How big it is", "That you've actually tested RESTORING from it",
               "Its file name", "That it runs at midnight",
               "B", "An untested backup is a hope; restore is the skill that counts."),
            _q("After restoring a deleted file, you must:",
               "Close the ticket", "Verify it: open it, confirm the right version and that permissions returned",
               "Delete the backup", "Reboot the server",
               "B", "Restore without verification is the verification-anchor failure."),
            _q("The 3-2-1 backup principle means:",
               "3 servers, 2 admins, 1 password", "3 copies, on 2 media types, with 1 offsite",
               "Back up 3 times daily", "3 GB minimum",
            ...
            _q("A nightly scheduled task stopped working right after a password change. Likely cause:",
               "The disk is full", "The task's run-as account now has stale credentials",
               "DNS failure", "The CPU is slow",
            ...
            _q("Invoke-Command -ComputerName s1,s2 { Get-Service DNS } does what?",
               "Installs DNS", "Runs the same command on multiple servers and returns objects",
               "Deletes the DNS service", "Reboots s1 and s2",
            ...
            _q("Before applying a patch to a production server, standard practice includes:",
               "Nothing special", "A maintenance window, a rollback plan (e.g. snapshot/backup), and post-patch verification",
               "Disabling the firewall", "Deleting old logs",
            ...
            _q("A junior tech's role in a full Domain Controller / AD system-state restore is to:",
               "Lead it solo", "Assist under senior involvement (authoritative vs non-authoritative is a big decision)",
               "Refuse to help", "Reinstall the DC instead",
