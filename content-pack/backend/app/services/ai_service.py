...
# --- AI provider configuration (TB-01 / TB-07) ---------------------------------
# Any OpenAI-compatible chat-completions endpoint is supported (OpenRouter, Ollama,
# vLLM, LM Studio...). New canonical vars: AI_BASE_URL, AI_MODEL, AI_API_KEY.
# Legacy OPENROUTER_* vars remain as fallbacks so existing deployments keep working.
# IMPORTANT: no import-time validation — the app must boot with AI unconfigured.
# Validation happens in _require_ai_configured(), called from call_ai().

OPENROUTER_API_KEY = os.getenv("AI_API_KEY", "").strip() or os.getenv("OPENROUTER_API_KEY", "").strip()
OPENROUTER_MODEL = os.getenv("AI_MODEL", "").strip() or os.getenv("OPENROUTER_MODEL", "").strip()
OPENROUTER_URL = os.getenv("AI_BASE_URL", "").strip().rstrip("/") or "https://openrouter.ai/api/v1"
if not OPENROUTER_URL.endswith("/chat/completions"):
    OPENROUTER_URL = OPENROUTER_URL + "/chat/completions"
OPENROUTER_SITE_URL = os.getenv("OPENROUTER_SITE_URL", "http://localhost:3000")
OPENROUTER_SITE_NAME = os.getenv("OPENROUTER_SITE_NAME", "Nexus Admin Academy")

# True when pointing at a local/self-hosted endpoint (Ollama needs no API key).
AI_IS_LOCAL = "openrouter.ai" not in OPENROUTER_URL
...
def ai_is_configured() -> bool:
    """AI is usable when a model is set and, for hosted providers, a key exists."""
    if not OPENROUTER_MODEL:
        return False
    if not AI_IS_LOCAL and not OPENROUTER_API_KEY:
        return False
    return True


def _require_ai_configured() -> None:
    """Call-time guard replacing the old import-time RuntimeError (TB-01)."""
    if not OPENROUTER_MODEL:
        raise HTTPException(
            status_code=503,
            detail="AI grading is not configured (set AI_MODEL / AI_BASE_URL). "
                   "Submissions are saved and can be mentor-graded manually.",
        )
    if not AI_IS_LOCAL and "/" not in OPENROUTER_MODEL:
        raise HTTPException(
            status_code=503,
            detail=f"Invalid AI model '{OPENROUTER_MODEL}' for hosted provider. Expected provider/model.",
        )
...
async def call_ai(
    *,
    ...
    if not AI_IS_LOCAL and not OPENROUTER_API_KEY:
        raise HTTPException(status_code=503, detail="AI API key not configured for hosted provider")
