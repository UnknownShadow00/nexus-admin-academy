...
QUIZZES_E = [
    {
        ...
        "questions": [
            ...
            _q("ls -l shows -rwxr-x---. The 'others' permission is:",
               "read+execute", "read+write", "nothing (---)", "full",
            ...
            _q("To add a user to a group WITHOUT removing their existing groups, use:",
               "usermod -G group user", "usermod -aG group user", "chmod group user", "chown user group",
            ...
            _q("chmod 777 on a web file to 'make it work' is bad because it:",
               "Is too slow", "Grants everyone full read/write/execute — the least-privilege violation",
               "Deletes the file", "Requires a reboot",
               "B", "777 is the Linux equivalent of Full Control to Everyone."),
            _q("On a DIRECTORY, the execute (x) permission means:",
               "You can run it as a program", "You can enter/traverse into it", "You can delete it", "Nothing",
            ...
            _q("You get 'Permission denied' editing a system file. The likely need (used carefully) is:",
               "chmod 777", "sudo (run as an elevated, audited command)", "reboot", "delete the file",
            ...
            _q("SSH refuses your key with 'Permission denied (publickey)'. A classic cause is:",
               "The server is off", "Wrong permissions on ~/.ssh (too open) or a missing authorized_keys entry",
               "DNS", "The firewall allows it",
    ...
    },
    {
        ...
        "questions": [
            _q("To see whether a service is running, failed, and enabled at boot, run:",
               "journalctl -f", "systemctl status <service>", "ps aux", "top",
            ...
            _q("A service is 'enabled' but currently 'inactive (dead)'. This means:",
               "It's running fine", "It will start at boot but is not running right now",
               "It's uninstalled", "It crashed permanently",
            ...
            _q("To read the actual error a failed service produced:",
               "ping it", "journalctl -u <service> -e", "chmod the binary", "reboot",
            ...
            _q("Restarting a failed service without reading its logs typically results in:",
               "A permanent fix", "It failing again for the same unaddressed reason",
               "Data loss", "A reboot",
            ...
            _q("A cron job 'works when I run it manually but not from cron'. The classic cause is:",
               "The disk is full", "cron's minimal environment/PATH — the script needs full paths",
               "DNS", "The user is locked",
            ...
            _q("To confirm WHEN a cron job was scheduled to run, you:",
               "Guess", "Read the crontab entry's five time fields", "Reboot", "Check the firewall",
    ...
    },
    {
        ...
        "questions": [
            _q("Before reloading nginx after a config change, you should run:",
               "nginx -reboot", "nginx -t (test the config)", "ufw disable", "rm the config",
            ...
            _q("A working nginx returns 403 Forbidden on a page. A common cause is:",
               "DNS", "Web-root PERMISSIONS (www-data can't read the files)", "A full disk", "Wrong DNS",
            ...
            _q("A service works locally on the server but is 'refused' from outside. Check FIRST:",
               "Reinstall the OS", "The firewall (ufw) rule for that port — don't just disable ufw",
               "The CPU", "DNS",
            ...
            _q("'Disk full' on Linux — the right way to find the cause is:",
               "Delete random files", "df -h to confirm, then du -sh /path/* to find what's consuming space",
               "Reboot", "chmod 777 /",
               "B", "Measure with df then du before deleting; often a runaway log in /var/log."),
            _q("An untested tar/rsync backup is:",
               "Perfectly safe", "A hope — you must restore and verify the data is intact",
               "Faster", "Encrypted",
            ...
            _q("An alert fires 'service X down'. A good first triage question is:",
               "Is it real or noise (flapping/maintenance), and what's the scope/impact?",
               "Who do I blame?", "Should I reboot everything?", "Can I ignore it?",
            ...
            _q("An alert says disk >90% and the service crashed. Restarting only the service will:",
               "Fix it permanently", "Likely crash again because the disk is still full (fix the cause)",
