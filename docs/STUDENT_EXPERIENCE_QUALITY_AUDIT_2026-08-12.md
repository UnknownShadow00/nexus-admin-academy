...
- **Pack gates:** Starter Support is available at Week 0. Desktop Support requires Week 3 and two Starter passes. Accounts & Access requires Week 6 and two Desktop passes. Networking requires Week 8 and two Accounts & Access passes. Advanced Troubleshooting requires Week 10 and three Networking passes. Packs unlock sequentially; XP is not an authority and every case in a pack is not required.
- **Assigned and Practice:** The highest unlocked pack's unpassed cases are Assigned. Successfully completed cases and earlier unlocked packs become Practice. A current in-progress attempt remains Assigned and replay creates a new version-bound attempt.
- **Curriculum alignment:** Required Service Desk activities now use `INC2405` (Week 1), `INC2404` (Week 2), `INC2501` (Week 3), `INC2509` (Week 4), `INC2502` (Week 5), `INC2505` (Week 6), `INC2507` (Week 7), `INC2407` (Week 8), and `INC2510` (Week 14). This supplies enough appropriately timed successes to reach each next pack without a dead end.
- **Assignment generation:** New admin-created students receive the managed 18-scenario assignment inventory, while the progression service decides which subset is visible and startable. Existing assignments and historical attempts remain intact.
- **Content versions:** Four solution-revealing scenarios (`INC2405`, `INC2406`, `INC2501`, and `INC2504`) receive new immutable published versions. Historical versions are not edited.
...
- Review the five prototype account cases for complete requester/device/tool/grading definitions before considering them for a future beginner pack.
