...
| `SERVICE_DESK_LAB_ENABLED` | Enable Service Desk student APIs | `false` | Disabled-by-default Phase 0 feature gate; no navigation is added in this phase |
...
Current web controls also include cookie `SameSite=Lax`, Origin/Referer CSRF
validation for state-changing cookie-authenticated requests, and security
response headers (CSP, HSTS, `X-Content-Type-Options`, `Referrer-Policy`,
`Permissions-Policy`, and `Cache-Control: no-store`) in FastAPI and nginx.
Ticket screenshot uploads use the same bounded-read policy as other evidence.
