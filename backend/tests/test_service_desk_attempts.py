...
def test_ownership_and_complete_is_idempotent_without_xp(db):
    ...
    before_xp = owner.total_xp; before_ledger = db.query(XPLedger).count()
    ...
    db.refresh(owner)
    assert first.status_code == 201 and second.status_code == 200 and second.json()["id"] == first.json()["id"]
    assert owner.total_xp == before_xp and db.query(XPLedger).count() == before_ledger
