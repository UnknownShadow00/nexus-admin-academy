...
def test_phase4c2_real_0059_and_fresh_paths_converge_without_identity_or_progress_churn(tmp_path):
    production_db = BACKEND_ROOT / "nexus.db"
    production_hash = _file_sha256(production_db)
    ...
    check = subprocess.run(
        ["git", "cat-file", "-e", f"{PRODUCTION_BASELINE_COMMIT}^{{commit}}"],
        cwd=REPO_ROOT,
        capture_output=True,
    )
    if check.returncode != 0:
        pytest.skip("the exact 0059 production baseline commit is not reachable")
    historical_path = tmp_path / "historical.db"
    historical_url = f"sqlite:///{historical_path}"
    shutil.copy2(production_db, historical_path)
    ...
    with Session(historical_engine) as db:
        ...
        db.add_all([run, cli_attempt, watch])
        db.commit()
        progress_ids = (student.id, run.id, cli_attempt.id, watch.id)
        assert next(
            item
    ...
    assert _active_totals(fresh_path) == _active_totals(historical_path) == (35, 320, 140, 180)
    ...
    assert _week_21_required_videos(fresh_path) == _week_21_required_videos(historical_path) == {"53"}
    ...
    assert _legacy_network_template_count(historical_path) == 0
    assert _active_totals(historical_path) == (35, 320, 140, 180)
    ...
    assert _canonical_target_state(fresh_path) == _canonical_target_state(historical_path)
    assert _active_totals(historical_path) == (35, 320, 140, 180)
    assert _file_sha256(production_db) == production_hash
...
def test_phase4c2_downgrade_restores_only_owned_0059_content(tmp_path):
    ...
    assert _active_totals(database_path) == (35, 320, 140, 180)
