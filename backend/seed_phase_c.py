...
QUIZZES_C = [
    {
        ...
        "questions": [
            _q("A host is 192.168.1.50/24 with gateway 192.168.2.1. Result:",
               "Works with the existing route", "Cannot reach its gateway",
               "Works only for local traffic", "Causes an address conflict",
            ...
            _q("A PC has mask 255.255.0.0 (/16) instead of the correct /24. The likely symptom:",
               "No effect on connectivity", "Remote hosts appear local",
               "Cannot get a DHCP lease", "Only DNS lookups fail",
            ...
            _q("Two hosts on the same subnet communicate by:",
               "Send every packet to gateway", "Use ARP and direct switching",
               "Perform DNS lookups first", "Use a static route",
    ...
    },
    {
        ...
        "questions": [
            ...
            _q("A user can ping local machines but nothing on other subnets. Most likely:",
               "DNS resolution failure", "Gateway or routing fault", "Physical cable fault", "Switch infrastructure outage",
            ...
            _q("A switch receives a frame for a destination MAC not in its table. It:",
               "Discards the unknown frame", "Floods it within the VLAN", "Logs an interface error", "Restarts forwarding process",
            ...
            _q("After a server's NIC was replaced, one client still can't reach it though others can. Suspect:",
               "A server service failure", "A stale ARP entry", "A DNS record problem", "A routing table failure",
            ...
            _q("A user can't ping ANYTHING, including devices on their own subnet. Layer to check first:",
               "DNS resolution layer", "Routing and default-gateway layer", "Physical and switching layer", "Application service layer",
    ...
    },
    {
        ...
        "questions": [
            _q("You just changed a switch config and it works. Before rebooting you must:",
               "Assume changes save automatically", "Save the running configuration",
               "Reload the switch software", "Erase saved configuration files",
            ...
            _q("A PC moved to a new port has no network; other PCs are fine. The port shows VLAN 1, should be VLAN 20. Fix:",
               "Replace the patch cable", "Assign VLAN 20 to the port",
               "Reload the switch configuration", "Reinstall the network adapter driver",
            ...
            _q("A switch port shows 'administratively down'. To bring it up:",
               "Replace the switch hardware", "Run no shutdown on the interface", "Change the access VLAN", "Save running configuration changes",
            ...
            _q("Which CLI mode do you configure a single port in?",
               "User EXEC command mode", "Interface configuration mode", "Privileged EXEC command mode", "Global configuration command mode",
            ...
            _q("Leaving user ports in VLAN 1 or using VLAN 1 for management is:",
               "Recommended default setup", "A security concern", "A required configuration", "An unsupported configuration",
            ...
            _q("A port shows 'err-disabled'. Running 'no shutdown' without more:",
               "Permanently fixes port-security state", "May re-disable if cause remains",
               "Removes the affected VLAN", "Resets all switch interfaces",
    ...
    },
    {
        ...
        "questions": [
            _q("A trunk port differs from an access port in that it:",
               "Carries one end-device VLAN", "Carries multiple tagged VLANs",
               "Always provides more bandwidth", "Cannot be configured manually",
            ...
            _q("Users in VLAN 20 on a newly-added switch B can't reach anything; VLAN 10 works. First check:",
               "Replace the new switch", "Verify VLAN 20 on both trunks",
               "Reload both switch configurations", "Reinstall workstation network drivers",
            ...
            _q("A native VLAN mismatch between two trunk ends causes:",
               "Complete trunk and VLAN connectivity failure", "Traffic leakage and mismatch warnings",
               "Higher trunk throughput", "A duplicate IP address",
            ...
            _q("VLAN 10 and VLAN 20 hosts need to communicate. This requires:",
               "A higher-capacity switch", "Inter-VLAN routing",
               "A shared patch cable", "Disabling both VLANs",
            ...
            _q("A whole new VLAN gets no IP (APIPA) but the DHCP server and other VLANs are fine. Likely cause:",
               "The DHCP server is down", "Missing DHCP relay",
               "All access cables failed", "DNS resolution failure",
               "B", "DHCP broadcasts don't cross routers; without a relay the new VLAN never reaches the server."),
            _q("show ip route is used to:",
               "List connected client devices", "View reachable networks",
               "Configure access VLANs", "Save switch configuration",
            ...
            _q("A host reaches its own VLAN but no other network. Besides gateway config, suspect:",
               "DNS resolution only", "A down SVI or router interface",
               "A failed patch cable", "An access-port security violation",
    ...
    },
    {
        ...
        "questions": [
            _q("Why manage switches over SSH rather than Telnet?",
               "SSH has lower latency", "SSH encrypts management traffic",
               "Telnet requires less configuration", "SSH uses less power",
            ...
            _q("A port is err-disabled. show port-security shows a violation. The RIGHT recovery:",
               "Run no shutdown before finding trigger", "Fix the violation, then re-enable",
               "Delete the affected VLAN", "Replace the whole switch",
            ...
            _q("Bottom-up troubleshooting: a user has no connectivity. You check FIRST:",
               "DNS resolution status", "Link, VLAN, and cabling", "The routing table", "The application service",
            ...
            _q("Why do network changes demand more caution than a single desktop fix?",
               "They have no additional operational risk", "Shared changes affect many users",
               "Switches are physically fragile", "Cisco policy requires caution",
            ...
            _q("The strongest network escalation note contains:",
               "A broad outage statement", "Scope and command evidence",
               "An untested root-cause guess", "A request to reboot everything",
            ...
            _q("You save a switch's config off-device primarily so that:",
               "It increases switching throughput", "Recover the configuration",
               "It enables all configured VLANs", "It activates SSH access",
