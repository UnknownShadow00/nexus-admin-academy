...
QUIZZES_F = [
    {
        ...
        "questions": [
            _q("A ticket concerns a broken OS service on an Azure IaaS VM. Who owns the fix?",
               "Microsoft cloud operations", "Your organization",
               "The internet service provider", "No responsible party",
            ...
            _q("In Azure's hierarchy, resources are organized as:",
               "Tenant, subscription, resource group, resource", "Region, user, file, service, account",
               "Domain, OU, GPO hierarchy", "Cluster, node, pod, workload, service",
               "A", "Tenant (identity) → subscriptions (billing) → resource groups → resources."),
            _q("An Entra 'can't log in' ticket should START with:",
               "Reset the password immediately", "Review Entra sign-in logs",
               "Re-enable the account", "Reinstall Microsoft Office",
            ...
            _q("A user lost their phone and asks you to reset their MFA. Before anything else you must:",
               "Reset MFA because they are stressed", "Verify their identity first",
               "Disable MFA permanently", "Provide an administrator phone",
            ...
            _q("In a hybrid (Entra Connect) setup, a synced user's password change usually must happen:",
               "In the cloud directory only", "In on-premises Active Directory",
               "On the user's mobile device", "Nowhere in the identity system",
            ...
            _q("'Password works on my laptop but not on webmail' in a hybrid org suggests:",
               "The user is reporting it incorrectly", "Check Entra Connect synchronization",
               "A failed network interface card", "A workstation Group Policy issue",
            ...
            _q("An account shows 'Block sign-in' enabled. Correct move:",
               "Unblock the account immediately", "Review why it was blocked",
               "Delete the affected account", "Reset all MFA methods",
    ...
    },
    {
        ...
        "questions": [
            _q("You can't SSH to a running Azure Linux VM. The FIRST cloud-layer check is:",
               "Reinstall the virtual machine", "Check the NSG rule",
               "Replace the user's keyboard", "Check laptop DNS settings",
            ...
            _q("A VM was stopped (deallocated) overnight; now 'its IP doesn't work'. Likely cause:",
               "An external attacker changed it", "The dynamic public IP changed",
               "The NSG rule was removed", "A stale DNS cache entry",
            ...
            _q("Opening RDP inbound from 'Any' source on an NSG is:",
               "Recommended security practice", "Unsafe internet-wide exposure",
               "A required Azure configuration", "Harmless for remote administration",
            ...
            _q("A wedged Azure VM won't respond at all. A cloud evidence source for 'what's on its screen' is:",
               "Azure activity log entries", "Boot diagnostics",
               "Storage account firewall rules", "Entra sign-in log records",
            ...
            _q("A shared blob link 'suddenly stopped working' for an external partner. A classic cause:",
               "The partner internet link failed", "The SAS token expired", "The storage VM restarted", "An MFA registration change",
            ...
            _q("'Storage works from the office but not from home' most likely involves:",
               "The user's local workstation", "Storage network access rules",
               "A full storage account disk", "A domain Group Policy setting",
            ...
            _q("'Who changed this NSG rule and when?' is answered by:",
               "Boot diagnostics screenshots", "Azure activity log", "ipconfig network output", "The VM event viewer logs",
