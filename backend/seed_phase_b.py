        ...
        "lessons": [
            ...
            },
            {
                ...
                "summary": (
                    ...
                    "reason, requested duration, approver named. A clean escalation note is a safe, complete outcome.\n\n"
        ...
        "lessons": [
            {
                ...
                "summary": (
                    ...
                    "turning it off and walking away leaves the workstation unsafe.\n\n"
        ...
        "lessons": [
            {
                ...
                "summary": (
                    ...
                    "   • 169.254.x.x → normal DHCP configuration was not obtained. Branch: is it just me (port/cable/NIC) or everyone "
                    ...
                    "2. ping <gateway> — CAN I LEAVE THE ROOM? Failure means reachability is not confirmed; ICMP may be blocked, so corroborate before naming the local path as the cause.\n"
                    "3. ping 1.1.1.1 — CAN I REACH THE INTERNET? If gateway reachability is confirmed but this fails, collect another relevant check before suspecting or escalating the router/upstream path.\n"
...
QUIZZES_B = [
    {
        ...
        "questions": [
            ...
            _q("Automatic Repair has failed twice in a loop. Best next move:",
               ...
               "Reinstall the operating system", "Clear firmware settings",
            ...
            _q("The repair ladder for a broken app, least destructive first:",
               ...
               "Reboot Windows, then reinstall", "Clear config, reinstall, restart",
               "B", "Each rung preserves more evidence and user config than the one below it."),
            _q("Before any startup fix is trusted, you should:",
               "Verify two successful boots", "Run Disk Cleanup", "Update system firmware", "Defragment the storage drive",
            ...
            _q("C: fills up again every week after cleanup. The real fix is:",
               "Schedule weekly cleanup", "Find and fix the disk writer",
               "Compress the volume", "Install a larger disk immediately",
    ...
    },
    {
        ...
        "questions": [
            _q("Share permission: Everyone Change. NTFS on the folder: user has Read. Over the network the user can:",
               "Read and write", "Read only", "No access at all", "Full control",
            ...
            _q("A user opens files fine AT the server console but is denied over the network. The gate at fault:",
               "NTFS file permissions", "Share permissions", "Group Policy assignment", "Network firewall rules",
            ...
            _q("You added a user to the right group but they're still denied. Most common missing step:",
               "Restart the file server", "Sign out and back in",
               "Disable folder inheritance", "Grant Full Control",
            ...
            _q("After a password change, a user gets 'the server is down' opening their mapped P: drive. Likely cause:",
               "Verify server availability", "Clear saved drive credentials",
               "Test DNS resolution", "Check whether the share was deleted",
            ...
            _q("A caller asks for a password reset 'urgently'. Before resetting you must:",
               "Ask a personal question", "Verify identity first",
               "Reset immediately", "Email the password anywhere",
            ...
            _q("A user reports their account is disabled. Correct first move:",
               "Re-enable the account immediately", "Review the disable reason",
               "Delete and recreate the account", "Reset the account password",
            ...
            _q("The request: contractor wants access to the Finance share 'for one urgent report'. Correct outcome:",
               "Grant temporary read access", "Escalate for approval",
               "Grant it and remove it tomorrow", "Refuse and close the request",
            ...
            _q("Recurring lockouts every ~30 minutes for one user usually mean:",
               "Brute-force attempts", "Stale saved credentials",
               "A weak password policy", "A domain controller outage",
    ...
    },
    {
        ...
        "questions": [
            _q("Defender detected and quarantined a trojan on a work PC that also shows odd outbound activity. Your move:",
               "Delete quarantine and clean up", "Isolate it and escalate",
               "Scan then close the ticket", "Reimage immediately",
            ...
            _q("Why keep an infected machine POWERED ON while isolating it?",
               "Complete the security scan first", "Shutdown loses volatile evidence",
               "Windows requires a live session", "Keep the user working normally",
            ...
            _q("A user entered their credentials on a fake login page an hour ago. The FIRST action:",
               "Run a full security scan", "Reset password and revoke sessions",
               "Delete the email and block its sender", "Restart and scan the computer",
            ...
            _q("A user REPORTS a phishing email they didn't click. Your response includes:",
               "Tell the user to be more careful", "Preserve and report the email",
               "Delete the email and close the ticket", "Open the link to test it",
            ...
            _q("'I turned the firewall off and the app worked.' As a resolution this is:",
               "Acceptable after documenting it", "A diagnostic finding, not a fix",
               "Preferred security practice", "A driver failure",
            ...
            _q("RDP to a workstation times out entirely (no error prompt). The four-gate ladder says start with:",
               "Check RDP Users membership", "Check network reachability first",
               "Check the RDP firewall rule", "Reinstall the remote desktop client",
            ...
            _q("For live support on a user's Windows 11 session while they watch, prefer:",
               "Use RDP to take over the session", "Use Quick Assist",
               "Install a random remote tool", "Use PowerShell remoting",
    ...
    },
    {
        ...
        "questions": [
            _q("ipconfig shows 169.254.40.7 on ONE machine; neighbors are fine. Most likely fault domain:",
               "DHCP scope is exhausted", "Local DHCP path failure",
               "DNS resolution failed", "Default gateway failure",
            ...
            _q("IP, gateway, DNS all look right; ping <gateway> ok; ping 1.1.1.1 fails. Escalate as:",
               "Configured DNS server failure", "Upstream routing failure",
               "DHCP lease failure", "Local cable fault",
            ...
            _q("nslookup google.com fails; nslookup google.com 1.1.1.1 succeeds. Conclusion:",
               "Internet routing failure", "Configured DNS problem",
               "Website access restriction", "ARP resolution failure",
            ...
            _q("IP 192.168.1.50/24 with gateway 192.168.2.1 will:",
               "Works at full speed", "Gateway is outside the subnet",
               "Works only for encrypted traffic", "Causes an address conflict",
            ...
            _q("'Fixing' a DHCP failure by assigning a static IP from the pool risks:",
               "No risk; this is standard practice", "Future DHCP address conflict",
               "Lower internet speed", "Recursive DNS lookups",
            ...
            _q("Jobs pile up in the queue; the printer's own panel shows a new IP since yesterday. The fix:",
               "Reinstall driver software", "Update the printer port",
               "Restart the workstation", "Replace the printer hardware",
            ...
            _q("Six tickets land at once. Your first five minutes go to:",
               "Investigate the oldest request first", "Triage every ticket first",
               "Work for the loudest user", "Work the easiest issue",
