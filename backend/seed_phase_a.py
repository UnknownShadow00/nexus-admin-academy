...
QUIZZES = [
    {
        ...
        "questions": [
            _q("An internal note reads: 'PC was broken, fixed it.' What is the FIRST thing missing?",
               "The technician's name", "What the symptom actually was", "The ticket close code", "An apology to the user",
               "B", "A note must let a stranger continue the work; without the symptom nothing else can be judged."),
            _q("Which sentence belongs in a USER-FACING resolution?",
               "Corrected HKCU ProfileImagePath and cleared the .bak SID key",
               "Your login profile had a glitch; your files are back and I confirmed they open",
               "User claimed files were 'gone' but obviously loaded a temp profile",
               "Escalating to T2 if reoccurs, see event 1511",
               "B", "User-facing text is plain language about outcome; registry paths and internal judgments stay in internal notes."),
            _q("The next tech should be able to continue your ticket without doing what?",
               "Reading your notes", "Re-contacting the user for the same information", "Checking the asset tag", "Logging in",
               "B", "That is the definition of a complete handoff: no re-asking the user."),
            ...
            _q("Blaming the user in writing ('user obviously deleted it') is bad primarily because:",
               ...
               "A", "Tickets are business records; keep them factual and professional."),
        ],
    },
    {
        ...
        "questions": [
            _q("A user logs in and their desktop is empty, documents 'gone', wallpaper default. Most likely:",
               "Ransomware encrypted the profile", "Windows loaded a temporary profile", "The disk failed", "Their account was deleted",
               "B", "The factory-fresh desktop is the classic temp-profile signature; files are still on disk under the original profile."),
            _q("Which Event Viewer events confirm a temp-profile logon?",
               "4624/4625", "1511/1515 (User Profile Service)", "7000/7001", "41/6008",
            ...
            _q("NTFS: a user is in GroupA (Modify ALLOW) and GroupB (Write DENY). Effective ability to save changes to a file?",
               "Can save; Allow wins", "Cannot save; explicit Deny overrides Allow", "Can save only as admin", "Depends on file size",
            ...
            _q("Password reset paths differ MOST between which two account types?",
               "Admin vs standard", "Local account vs Microsoft account", "Guest vs standard", "32-bit vs 64-bit",
            ...
            _q("Permissions on C:\\Share\\Reports are inherited from:",
               "The domain controller", "C:\\Share (its parent), unless inheritance is disabled", "The user's profile", "The BIOS",
            ...
            _q("Before demonstrating a permissions fix on a user's folder you should FIRST:",
               "Take ownership", "Confirm a backup / know how to revert the ACL", "Disable inheritance", "Reboot",
               "B", "Rollback thinking: never change what you can't restore."),
            _q("A 'corrupt profile' usually means the user's files are:",
               "Encrypted", "Deleted", "Still on disk under the original profile path", "Moved to OneDrive",
            ...
            _q("Which tool shows the effective result of all group memberships on a folder for one user?",
               "Effective Access tab in Advanced Security Settings", "Task Manager", "Disk Management", "msconfig",
    ...
    },
    {
        ...
        "questions": [
            ...
            _q("A service is set to Automatic but shows Stopped after boot. Best next step:",
               "Reinstall Windows", "Check its entry in Event Viewer System log for the failure reason",
               "Set it to Manual", "Delete the service",
            ...
            _q("Which pairs a network connection to the program that owns it?",
               "ipconfig /all", "netstat -ano plus Task Manager Details (PID)", "tracert", "gpresult /r",
            ...
            _q("Login takes 4 minutes. The fastest evidence source is:",
               "Task Manager → Startup impact", "Disk Management", "nslookup", "Windows Update history",
               "A", "Startup impact ranks exactly what slows logon."),
            _q("Event Viewer 'Level' tells you:",
               "Severity (Error / Warning / Information)", "Which user caused it", "The fix", "CPU usage",
            ...
            _q("Windows is 'out of space' but the user 'deleted everything'. First evidence to collect:",
               "Disk Management + a folder-size view of C: (what is actually consuming space)",
               "ipconfig /all", "gpresult", "A reboot",
            ...
            _q("Stopping vs killing: the correct way to restart a hung Automatic service is:",
               "End task on its process tree in Task Manager", "services.msc → Restart (or net stop/start), reading errors if it fails",
               "Rename its exe", "Reboot immediately",
    ...
    },
    {
        ...
        "questions": [
            _q("ipconfig shows 169.254.23.7. This means:",
               "Static IP set", "DHCP failed; Windows self-assigned (APIPA)", "DNS is down", "The NIC is disabled",
            ...
            _q("Correct repair order for system file corruption:",
               "chkdsk then format", "DISM /Online /Cleanup-Image /RestoreHealth then sfc /scannow",
               "sfc alone, always twice", "Defrag then sfc",
            ...
            _q("You must NEVER run chkdsk /f on:",
               "An SSD", "A drive making mechanical clicking sounds", "The C: drive", "A USB stick",
            ...
            _q("gpresult /r answers which question?",
               "Which Group Policy objects applied to this user/machine", "Which ports are open",
               "Which updates failed", "Which DNS server is set",
...
QUIZZES.append({
    ...
    "questions": [
        _q("A VIP's personal printer is jammed; a 40-person department share is down; a password reset waits. Correct order:",
           "VIP, share, reset", "Share outage, password reset, VIP printer", "Reset, VIP, share", "First come, first served",
        ...
        _q("A ticket asks you to grant a contractor access to the HR folder 'urgently'. Correct outcome:",
           "Grant it; it's urgent", "Escalate for approval — sensitive access needs authorization, urgency doesn't change that",
           "Grant read-only as a compromise", "Close the ticket",
           "B", "Sensitive-data access requires the approval path; correct escalation scores full marks on safe_fix_or_escalation."),
        ...
        _q("You promised the user an update by 3 PM but have no fix yet. At 3 PM you should:",
           "Wait until you have good news", "Send the update anyway: status, what you're doing, next update time",
           "Close and reopen the ticket", "Escalate to avoid the conversation",
           "B", "Honest motion beats silence; missed updates destroy trust faster than slow fixes."),
    ],
})
