...
from app.models.ai_rate_limit import AIRateLimit  # noqa: E402
...
async def main() -> int:
    ...
    failures = 0
    try:
        # Calibration runs as user_id=0 (mentor-only offline tool) and needs
        # 5 grading calls per run — clear its own counters so back-to-back
        # runs don't trip the 8/day ticket_grading cap meant for students.
        db.query(AIRateLimit).filter(
            AIRateLimit.user_id == 0, AIRateLimit.endpoint == "ticket_grading"
        ).delete()
        db.commit()
        first = True
        for name, writeup in FIXTURES.items():
            if not first:
                await asyncio.sleep(25)  # space out calls so calibration doesn't trip the AI rate limit
            first = False
            try:
